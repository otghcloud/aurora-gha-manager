...
/**
 * Validates runner template metadata, credentials, and node mappings.
 */
class RunnerTemplateRequest extends FormRequest
{
    /** Allow authenticated administrators to submit template changes. */
     ...
     */
    /** @return array<string, mixed> Validation rules for template configuration. */
    ...
    /** Resolve catalog values and credential defaults before validation. */
    protected function prepareForValidation(): void
    {
        $entry = app(TemplateCatalog::class)->entryForId($this->input('template_catalog_id'));

        $credentialId = $this->input('credential_id');
        if ($credentialId === null && ($entry?->platform() === PoolOs::Linux->slug() || $this->input('os') === (string) PoolOs::Linux->value)) {
            $credentialId = Credential::query()->where('name', 'Default Linux SSH')->value('id');
        }

        $this->merge([
            'name' => $entry?->name(),
            'os' => $entry === null ? $this->input('os') : PoolOs::fromSlug($entry->platform())->value,
            'credential_id' => $credentialId,
        ]);
    }

    /** Add catalog, credential, and mapping consistency checks. */
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($credential === null || $credential->os !== PoolOs::tryFrom((int) $this->input('os'))) {

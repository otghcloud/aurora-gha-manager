...
/**
 * Executes an image build outside the request lifecycle.
 */
class BuildImageJob implements ShouldQueue
{
    ...
    /** @param int $imageBuildId Identifier of the build to execute. */
    public function __construct(public readonly int $imageBuildId)
    {
        $this->onQueue('builds');
    }

    /** Execute the build and persist any failure state. */
    ...
    /** Mark the build failed after an unrecoverable queue error. */

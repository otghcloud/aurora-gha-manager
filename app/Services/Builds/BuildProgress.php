...
/**
 * Calculates display progress and lifecycle messages for image builds.
 */
class BuildProgress
{
    ...
    private function seenStageIds(ImageBuild $build): array
    {
        ...
        // Stage ids are usually kebab-case, but at least one upstream-derived id
        // (`Install-PowerShellModules`) is mixed case - a lowercase-only pattern here silently
        // left it permanently "pending" since its marker could never match.
        preg_match_all('/\[image-builder:stage:([A-Za-z0-9-]+)\]/', $contents, $matches);

...
/**
 * Provisions, configures, and registers runner VMs for a pool.
 */
class Provisioner
{
     ...
     */
    /** @throws CapacityException|ProvisioningException When a runner cannot be provisioned. */
    ...
    /** Return the target selected during the current provisioning operation. */
    ...
    private function build(Runner $runner, Pool $pool): void
    {
        ...
        $this->awaitFirstBoot($credential, $template->os, $ip, $pool->boot_timeout_seconds);
    ...
    /**
     * Wait until first-boot configuration has finished before using the guest.
     *
     * An open SSH port only means sshd is accepting connections, not that the guest has settled.
     * Templates are sealed with `cloud-init clean` and no host keys, so on first boot cloud-init
     * regenerates them and restarts sshd - which drops any session already established, surfacing
     * as "Connection closed by server" partway through the launch rather than as a failed login.
     */
    private function awaitFirstBoot(Credential $credential, PoolOs $os, string $host, int $timeout): void
    {
        if ($os !== PoolOs::Linux) {
            return;
        }
        ...
        while (microtime(true) < $deadline) {
            // A fresh connection each poll: if sshd restarts mid-check the attempt simply fails
            // and the next one reconnects, which is the condition being waited out.
            ...
            try {
                $status = trim($ssh->run('cloud-init status 2>/dev/null || echo "status: unknown"'));
                $lastStatus = $status;

                // `running` is the only state worth waiting on; `error` still leaves a usable
                // guest, and `unknown` means the image has no cloud-init to wait for.
                if (! str_contains($status, 'status: running')) {
                    return;
                }
            } catch (Throwable $e) {
        ...
        throw new ProvisioningException(
            "Guest {$host} never finished cloud-init within {$timeout}s (last status: ".($lastStatus ?? 'none').').'
        );
    }

...
/**
 * Reconciles tracked runners with Proxmox and GitHub before cleanup.
 */
class Reaper
{
    /** Grace period before a runner that never registered is considered abandoned. */
    private const REGISTRATION_GRACE_SECONDS = 180;
     ...
     */
    /** @return int Number of runners expected to be destroyed. */
    ...
    private function destructionReason(Runner $runner, ?GitHubRunner $githubRunner): ?string
    {
        ...
        if (! $githubRunner->isOnline()) {
            $runnerAge = $runner->ageSeconds();

            // A freshly booted runner can briefly look offline while GitHub is settling the JIT
            // registration or the guest is still finalizing its first boot. Keep it alive until the
            // normal runner-registration grace window has passed before treating it as dead.
            if ($runnerAge <= self::REGISTRATION_GRACE_SECONDS) {
                return null;
            }

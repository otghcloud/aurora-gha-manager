...
class TemplateDownloadService
{
    ...
    /**
     * Adopt the image's bundled templates when they are newer than the pinned download.
     *
     * The active version is a stored setting, so upgrading the container leaves it pointing at
     * whatever was downloaded before - the newer templates shipped in the new image would sit
     * installed but unused until someone pressed "Download now". Called on boot.
     *
    ...
    public function adoptBundledIfNewer(): ?string
    {
        ...
        $bundled = $this->bundledEntry(false);

        if ($bundled === null || version_compare($bundled['version'], $active) <= 0) {
            return null;
        }

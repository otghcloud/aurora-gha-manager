...
<div data-node-form data-storage-url="{{ route('nodes.storage-options') }}" data-is-update="{{ $isUpdate ? 'true' : 'false' }}" data-target-id="{{ $target->id ?? '' }}">
	<div class="card">
		...
		<div class="card-body"><div class="row g-3">
			...
			@php($authRealm = old('proxmox_auth_realm', $target->proxmox_auth_realm ?? \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_API_TOKEN))
			<div class="col-md-12"><label class="form-label required" for="proxmox_auth_realm">Authentication method</label>
				<select class="form-select" data-auth-realm id="proxmox_auth_realm" name="proxmox_auth_realm" required>
					<option value="{{ \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_API_TOKEN }}" @selected($authRealm === \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_API_TOKEN)>API authentication</option>
					<option value="{{ \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_PASSWORD }}" @selected($authRealm === \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_PASSWORD)>Standard authentication</option>
				</select>
			...
			<div data-auth-fields="{{ \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_API_TOKEN }}" class="col-md-6"><label class="form-label" for="proxmox_token_id">API token ID</label><input class="form-control" id="proxmox_token_id" name="proxmox_token_id" type="text" value="{{ old('proxmox_token_id', $target->proxmox_token_id) }}"></div>
			<div data-auth-fields="{{ \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_API_TOKEN }}" class="col-md-6"><label class="form-label" for="proxmox_token_secret">API token secret</label><input autocomplete="new-password" class="form-control" id="proxmox_token_secret" name="proxmox_token_secret" type="password" value="{{ old('proxmox_token_secret', $target->proxmox_token_secret) }}"></div>
			<div data-auth-fields="{{ \App\Models\Infrastructure\ProxmoxTarget::AUTH_REALM_PASSWORD }}" class="col-md-6"><label class="form-label" for="proxmox_username">Username</label><input class="form-control" id="proxmox_username" name="proxmox_username" type="text" placeholder="root@pam" value="{{ old('proxmox_username', $target->proxmox_username) }}"></div>

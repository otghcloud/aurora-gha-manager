...
class BuildProgressTest extends TestCase
{
    ...
    public function test_a_mixed_case_stage_id_is_still_recognised(): void
    {
        // Real-world regression: `Install-PowerShellModules` (mixed case, unlike every other
        // stage id) was silently stuck as "pending" forever because the marker regex only
        // matched lowercase ids.
        file_put_contents($this->directory.'/templates/ubuntu/ubuntu2404/packer/build.json', json_encode([
            'stage_groups' => [
                ['id' => 'install', 'display_order' => 1, 'stages' => [
                    ['id' => 'Install-PowerShellModules', 'name' => 'Install PowerShell and Azure modules', 'marker' => '[image-builder:stage:Install-PowerShellModules]'],
                ]],
            ],
        ], JSON_THROW_ON_ERROR));

        $log = $this->directory.'/build.log';
        file_put_contents($log, "[image-builder:stage:Install-PowerShellModules] Install PowerShell and Azure modules\n");
        ...
        $this->assertSame(['current'], array_column($progress['stages'], 'state'));
    }
}

...
class ReaperDecisionsTest extends TestCase
{
    ...
    public function test_a_runner_within_the_initial_registration_grace_is_not_reaped_for_offline_status(): void
    {
        $runner = Runner::create([
            ...
            'vmid' => 905,
            'runner_name' => 'gha-ubuntu-slim-905-abc',
            'state' => RunnerState::Idle,
            'state_changed_at' => now()->subSeconds(30),
            'created_at' => now()->subSeconds(30),
            'source_template_vmid' => 801,
        ]);

        $this->assertSame(0, $this->reaperForOffline($runner)->pendingCount());
        $this->assertNull($this->reasonForStatus($runner, 'offline'));
    }
    ...
    {
        return $this->reasonForStatus($runner, 'online', $busy);
    }

    private function reasonForStatus(Runner $runner, string $status, bool $busy = false): ?string
    {
        $method = new ReflectionMethod(Reaper::class, 'destructionReason');

        return $method->invoke($this->reaper(), $runner, new GitHubRunner(1, $runner->runner_name, $status, $busy));
    }
    ...
    private function reaperForOffline(Runner $runner): Reaper
    {
        $github = $this->createMock(GitHubClient::class);
        $github->method('listRunners')->willReturn([
            $runner->runner_name => new GitHubRunner(1, $runner->runner_name, 'offline', false),
        ]);

        return new Reaper(
            $this->environment,
            $this->target,
            $this->proxmoxReporting([$runner->vmid]),
            $github,
            $this->createMock(Provisioner::class),
        );
    }

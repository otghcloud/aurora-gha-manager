...
class RunnerImagesLocatorTest extends TestCase
{
    ...
    public function test_filesystem_root_resolves_the_bundled_tree(): void
    {
        $this->writeCatalog('259c79eaf7bb3dde3d88a843bc4a4c57ea342d20');
        mkdir($this->directory.'/vendor/runner-images/images/ubuntu-slim', 0755, true);

        $this->assertSame(
            $this->directory.'/vendor/runner-images/images/ubuntu-slim',
            (new RunnerImagesLocator)->filesystemRoot($this->ubuntuSlim()),
        );
    }

    public function test_filesystem_root_returns_null_when_scripts_are_not_required(): void
    {
        $this->writeCatalog(null);

        $this->assertNull((new RunnerImagesLocator)->filesystemRoot(new TemplateCatalogEntry([
